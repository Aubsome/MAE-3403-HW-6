...
class steam:
    ...
    def calc(self):
        ...
        ts, ps, hfs, hgs, sfs, sgs, vfs, vgs = np.loadtxt('saturated_properties.txt', unpack=True)  # use np.loadtxt to read the saturated properties
        tcol, hcol, scol, pcol = np.loadtxt('superheated_properties.txt', unpack=True)  # use np.loadtxt to read the superheated properties
        ...
        elif self.x is not None:  # manual interpolation
